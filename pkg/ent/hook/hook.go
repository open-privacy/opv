...
// function as APIAudit mutator.
type APIAuditFunc func(context.Context, *ent.APIAuditMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f APIAuditFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.APIAuditMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.APIAuditMutation", m)
	}
	return f(ctx, mv)
}
...
// Mutate calls f(ctx, m).
func (f FactFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.FactMutation)
	if !ok {
